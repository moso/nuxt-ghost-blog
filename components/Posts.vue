            ...
            <div class="row">
                <div class="md-11 lg-10 mx-auto">
                    <div class="post-content">
                        <div v-if="post.tags.length > 0" class="post-tags">
                            <ul class="list inline">
                                <li v-for="tag in post.tags" :key="tag.id">
                                    <nuxt-link :to="{ path: '/tag/' + tag.slug }" :title="tag.name" class="button tag-button">{{ tag.name }}</nuxt-link>
                                </li>
                            </ul>
                        </div>
                        <h1 v-if="post.title" class="post-title"><nuxt-link :to="{ path: '/' + post.slug }" :title="post.title">{{ post.title }}</nuxt-link></h1>
                        <div class="post-author">
                            <div v-if="post.primary_author.profile_image" class="post-author-avatar">
                                <nuxt-link :to="{ path: '/author/' + post.primary_author.slug }" :title="post.primary_author.name">
                        ...
                        <p v-else-if="post.excerpt && !post.custom_excerpt">{{ post.excerpt }}</p>
                        <nuxt-link class="post-read-more" :to="{ path: '/' + post.slug }" title="Read more...">Read more...</nuxt-link>
                        <div class="post-meta">
            ...
            <div class="md-11 lg-10 mx-auto">
                <div class="pagination">
                    <div class="prev" :class="{ 'disabled' : !pagination.prev }">
                        <nuxt-link :to="prevLink" class="pagination-link" title="Newer posts">Newer posts</nuxt-link>
                    </div>
                    <div class="next" :class="{ 'disabled' : !pagination.next }">
                        <nuxt-link :to="nextLink" class="pagination-link" title="Older posts">Older posts</nuxt-link>
                    </div>
                </div>
            </div>
        </div>
